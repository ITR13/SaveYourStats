...
public class EnemyManager : MonoBehaviour
{
    ...
    public void SpawnEnemies(int difficulty, int count)
    {
        ...
        {
            var index = i;
            _enemies.Add(Instantiate(_enemyPrefabs[difficulty], _spawningPositions[i]));
            _enemies[^1].OnClick = () => _lastClickedEnemy = index;
        }
    }
    ...
    public IEnumerator EnemyTurn()
    {
        for (var i = 0; i < _enemies.Count; i++)
        {
            ...
            yield return StartCoroutine(_player.DamagePlayer(enemy.Damage));
        }
    }
